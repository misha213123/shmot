...
const products: Product[] = [
  { id: 1, brand: 'STONE ISLAND', title: 'Чёрная куртка с капюшоном', size: 'L', price: '21 900 ₽', city: 'Москва', condition: '9/10', image: 'https://images.unsplash.com/photo-1551488831-00ddcb6c6bd3?auto=format&fit=crop&w=1000&q=85', likes: 312 },
  { id: 2, brand: 'NEW BALANCE', title: 'Кроссовки 530', size: '43', price: '14 900 ₽', city: 'Санкт-Петербург', condition: '8/10', image: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=1000&q=85', likes: 208 },
  { id: 3, brand: 'CARHARTT', title: 'Винтажная рабочая куртка', size: 'M', price: '18 900 ₽', city: 'Минск', condition: '9/10', image: 'https://images.unsplash.com/photo-1591047139829-d91aecb6caea?auto=format&fit=crop&w=1000&q=85', likes: 174 },
  { id: 4, brand: 'NIKE', title: 'Худи оверсайз', size: 'XL', price: '8 900 ₽', city: 'Москва', condition: 'Новое', image: 'https://images.unsplash.com/photo-1556821840-3a63f95609a7?auto=format&fit=crop&w=1000&q=85', likes: 147 },
];
...
export default function AppStable() {
  ...
  const renderExplore = () => <>
    ...
    <div className="product-grid">{filtered.map((item) => <button key={item.id} className="grid-item pressable-card" onClick={() => { setIndex(products.findIndex((p) => p.id === item.id)); navigate('product'); }}><div className="grid-image-wrap"><img src={item.image} alt={item.title} /></div><b>{item.price}</b><span>{item.brand}</span></button>)}</div>
  </>;

  const renderProduct = () => <>{header('Товар', true)}<div className="detail-image-wrap"><img className="detail-image" src={product.image} alt={product.title} /></div><section className="detail-card"><div className="detail-title"><div><b>{product.brand}</b><h1>{product.title}</h1></div><button className="icon-button" onClick={() => toggleLike(product.id)}><Heart fill={liked.includes(product.id) ? 'currentColor' : 'none'} /></button></div><strong className="detail-price">{product.price}</strong><div className="spec-grid"><span><small>Размер</small>{product.size}</span><span><small>Состояние</small>{product.condition}</span><span><small>Город</small>{product.city}</span></div><button className="primary-button" onClick={() => navigate('messages')}><MessageCircle size={19} /> Написать продавцу</button></section></>;
  ...
  const renderCreate = () => <>{header('Новое объявление', true)}<form className="form" onSubmit={(e) => { e.preventDefault(); showNotice('Объявление сохранено'); navigate('profile'); }}><button type="button" className="upload">＋<b>Добавить фотографии</b></button><label>Название<input required /></label><label>Бренд<input required /></label><div className="form-row"><label>Размер<input /></label><label>Цена<input type="number" /></label></div><label>Описание<textarea rows={5} /></label><button className="primary-button" type="submit">Опубликовать</button></form></>;
  const renderProfile = () => <>{header('Профиль', true)}<section className="profile-head"><div className="avatar large">Д</div><h2>drip.collector ✓</h2><p>Москва, Россия</p></section><div className="product-grid">{products.map((item) => <button key={item.id} className="grid-item"><img src={item.image} alt={item.title} /><b>{item.price}</b><span>{item.title}</span></button>)}</div></>;
