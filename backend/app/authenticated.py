...
async def require_profile(user: AuthUser, session: AsyncSession) -> Profile:
    ...
    if profile is None:
        raise HTTPException(
            ...
            detail="Сначала заполните профиль",
        )
    return profile
...
async def upsert_my_profile(
    ...
    username_owner = await session.scalar(
        select(Profile).where(Profile.username == payload.username, Profile.id != user.id)
    )
    if username_owner:
...
async def favorite_product(
    ...
    existing = await session.scalar(
        select(Favorite).where(Favorite.user_id == profile.id, Favorite.product_id == product.id)
    )
    if existing is None:
...
async def swipe_product(
    ...
    existing = await session.scalar(
        select(SwipeAction).where(
            SwipeAction.user_id == profile.id,
            SwipeAction.product_id == product.id,
        )
    )
    if existing:
